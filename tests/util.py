"""Minimal pure-Python regtest block miner for integration tests.

Regtest's proof-of-work target (nBits 0x207fffff) is trivial, so grinding
a valid nonce takes only a couple of hash attempts per block. Blocks
contain a coinbase paying 50 BTC to OP_TRUE plus any extra transactions
...
def mine_chain(genesis_hash: bytes, num_blocks: int) -> list[bytes]:
    """Mine num_blocks empty blocks on top of the genesis hash."""
    blocks = []
    prev = genesis_hash
